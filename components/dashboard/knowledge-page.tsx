...
export function KnowledgePage() {
  ...
  return (
    <div className="space-y-8">
      <header><p className="text-sm font-semibold uppercase tracking-[0.16em] text-violet-600">Knowledge</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.04em]">Make retrieval trustworthy.</h1><p className="mt-3 max-w-2xl text-slate-500">Keep manual, workspace-isolated knowledge clear while indexing status remains visible and safe. Start with product facts, delivery areas, or brand guidelines.</p></header>
      {message ? <InlineAlert tone={message.includes("could not") ? "error" : "success"}>{message}</InlineAlert> : null}
      {!selectedWorkspace ? <EmptyState title="Create a workspace first" description="Knowledge belongs to a workspace and brand. Start from the Brands page." /> : brands.length === 0 ? <EmptyState title="Add a brand first" description="Create a brand before adding bounded knowledge. Then add short, trusted business facts here." /> : <>
