...
export function WorkflowPanel() {
  ...
  async function loadWorkflows(nextWorkspaceId: string) {
    ...
    setSelectedWorkflowId((current) => body.workflows.some((workflow) => workflow.id === current) ? current : body.workflows[0]?.id ?? null);
  }
  ...
  useEffect(() => { setSelectedWorkflowId(null); void loadWorkflows(workspaceId).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : "Could not load workflows.")); }, [workspaceId]);
