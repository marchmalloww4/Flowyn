...
export function canPerformWorkspaceAction(role: WorkspaceRole, action: WorkspaceAction): boolean {
  ...
  return ["workspace.read", "brand.read", "agent.read", "agent.run", "workflow.read", "workflow.run", "workflow.cancel", "workflow_schedule.read", "workflow_webhook.read"].includes(action);
}
